...
import ProviderSelectMenuItem from './provider-select-menu-item.js';

/**
 * A `ProviderSelect` class for any popup {@link Menu}.
 *
 * @extends Component
 */
class ProviderSelect extends Component {
  ...
  constructor(player, options = {}) {
    ...
    this.on(player, 'providersloaded', (e, providers) => this.update(providers));
  }
  ...
  createItems() {
    ...
    for (let i = this.providers.length - 1; i >= 0; i--) {
      items.push(new ProviderSelectMenuItem(this.player(), {provider: this.providers[i]}));
    }
  ...
  /**
   * Create the `ProviderSelects`s DOM element.
   *
   * @return {Element}
  ...
  createEl() {
    ...
    el.appendChild(Dom.createEl('img', {
      className: 'ocx-provider-icon'
    }));
    el.appendChild(Dom.createEl('div', {
      className: 'ocx-provider-name',
      textContent: "Select Provider",
      tabIndex: -1
    }));
  ...
  buildWrapperCSSClass() {
    let menuButtonClass = 'ocx-provider-select';
    ...
    return `ocx-provider-select ${menuButtonClass} ${super.buildCSSClass()}`;
  }
  ...
  /**
   * Handle a click on a `ProviderSelect`.
  ...
  /**
   * Handle `mouseleave` for `ProviderSelect`.
   *
   * @param {Event} event
  ...
  /**
   * Handle tab, escape, down arrow, and up arrow keys for `ProviderSelect`. See
  ...
  /**
   * Handle a `keyup` event on a `ProviderSelect`. The listener for this is added in
   * the constructor.
   *
   * @param {Event} event
   *        Key press event
  ...
  /**
   * Put the current `ProviderSelect` into a pressed state.
   */
  pressButton() {
    if (this.enabled_) {
  ...
  /**
   * Take the current `ProviderSelect` out of a pressed state.
   */
  unpressButton() {
    if (this.enabled_) {
      this.buttonPressed_ = false;
  ...
  /**
   * Disable the `ProviderSelect`. Don't allow it to be clicked.
   */
  disable() {
    this.unpressButton();
  ...
  /**
   * Enable the `ProviderSelect`. Allow it to be clicked.
   */
  enable() {
    this.enabled_ = true;
